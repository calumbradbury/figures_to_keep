...
def getAxis(column):
    ...
    with open(target+source,'r') as csvfile:
        ...
        for x,y in zip(range(0,150,25),range(25,175,25)):   
            selectedDF = df[df["second_inv"] >= x]
...
x = np.arange(len(range(0,150,25)))
ys = [i+x+(i*x)**2 for i in range(len(range(0,150,25)))]
colours = cm.rainbow(np.linspace(0, 1, len(ys)))

bins = [str(y)+'-'+str(z) for y,z in zip(range(0,150,25),range(25,175,25))]
...
leg = plt.legend(handles=legends, title='Strain (25 bins)',ncol=7,loc='upper center', bbox_to_anchor=(0.5, 1.05)) 
