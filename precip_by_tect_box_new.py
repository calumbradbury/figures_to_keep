...
def getAxisBox(column,mins,maxs):
    ...
    with open(target+source,'r') as csvfile:
        ...
        for x,y in zip(litho_mins,litho_maxs):
            ...
            selectedDF_a = selectedDF_a[selectedDF_a['tectonics'] < y]            
...
ax.set_xticklabels(new_names,rotation=90,fontsize='small')
...
plt.subplots_adjust(wspace=0, hspace=0,bottom=0.18)
#fig.tight_layout()
...
leg = fig.legend(legends, names_c,'lower center',ncol=6,title=r'Precipitation ($mm\,yr^{-1}$)') 



fig.savefig('../precip_by_tect_0.4_removed.png',bbox_inches='tight')
